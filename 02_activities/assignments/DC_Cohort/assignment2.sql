...
SELECT * FROM (SELECT *, DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date DESC) AS visit_rank_desc FROM customer_purchases) AS ranked_visits
WHERE ranked_visits.visit_rank_desc = 1;
...
SELECT 
    v.vendor_name,
    p.product_name,
    vi.original_price * 5 * customer_count.total_customers AS total_potential_revenue
FROM vendor_inventory AS vi
JOIN vendor AS v ON vi.vendor_id = v.vendor_id
JOIN product AS p ON vi.product_id = p.product_id
CROSS JOIN (SELECT COUNT(customer_id) AS total_customers FROM customer) AS customer_count
...
VALUES (99, 'Apple Pie', '1 pie', 'unit', CURRENT_TIMESTAMP);
